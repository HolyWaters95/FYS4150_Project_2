...
int main(){
    ...
    // Run segment for all values of N and W
    for(uword j = 0;j<omegas.n_elem;j++){
        double w = omegas(j);
        int Xmax = 0;
      for (int i=0;i<N_values.size();i++){

        // Setting up boundary conditions and step length
        if (w == omegas(0)){Xmax = 30;}
        else{Xmax = 10;}
        int N = N_values[i];
        ...
        if (save_eigenpairs == "y"){
            string filenameeigen = "Quantum_Eigenpairs_1particle_N_" + to_string(N) + "_X_" + to_string(Xmax) + "_w_" + to_string(w) + ".txt";
            ofstream output;
    ...
    // Save number of transformations to file
